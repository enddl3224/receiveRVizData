...
AutowareStatePanel::AutowareStatePanel(QWidget * parent) : rviz_common::Panel(parent)
{
  ...
  // EDIT_ :ADD "Send Loop Limit" Button
  loop_limit_button_ptr_ = new QPushButton("Send Loop Limit");
  pub_loop_limit_input_ = new QSpinBox();
  pub_loop_limit_input_->setRange(0, 100);
  pub_loop_limit_input_->setValue(0);
  pub_loop_limit_input_->setSingleStep(1);
  connect(loop_limit_button_ptr_, SIGNAL(clicked()), this, SLOT(onClickLoopLimit()));
  ...
  // EDIT_ :Settings the "Send Limit Loop" Button layout on RViz
  auto * loop_limit_layout = new QHBoxLayout();
  loop_limit_layout->addWidget(loop_limit_button_ptr_);
  loop_limit_layout->addWidget(pub_loop_limit_input_);
  loop_limit_layout->addWidget(new QLabel("  [loops]"));
  v_layout->addLayout(loop_limit_layout);
...
void AutowareStatePanel::onInitialize()
{
  ...
  // EDIT_ :Setting topic name
  pub_loop_limit_ = raw_node_->create_publisher<std_msgs::msg::UInt8>(
    "/planning/mission_planning/loop_limit", rclcpp::QoS{1}.transient_local());
}
...
// EDIT_ :Publish as UInt8 msg when "Send Loop Limit" Button is clicked
void AutowareStatePanel::onClickLoopLimit()
{
  auto loop_limit = std::make_shared<std_msgs::msg::UInt8>();
  loop_limit->data = pub_loop_limit_input_->value();
  pub_loop_limit_->publish(*loop_limit);
}
